...
## ----------------------------------Internal links---------------------------------------## ----
  ...
  observeEvent(input$more_info_button, {
    ...
    navpage_name <- "Model + Data"
    updateNavbarPage(session, "pages",navpage_name)
    updateTabsetPanel(session, "model_data_panels", panel_name)
  })
  ...
  observeEvent(input$link_from_spatial_to_watersheds1, {
    navpage_name <- "Watersheds"
    updateNavbarPage(session, "pages",navpage_name)
  })
  ...
  observeEvent(input$link_from_spatial_to_watersheds4, {
    navpage_name <- "Watersheds"
    updateNavbarPage(session, "pages",navpage_name)
  })
  ...
  observeEvent(input$link_from_spatial_to_framework_overview2, {
    panel_name <- "Framework Overview"
    navpage_name <- "Model + Data"
    updateNavbarPage(session, "pages",navpage_name)
    updateTabsetPanel(session, "model_data_panels", panel_name)
  ...
  observeEvent(input$link_from_wat_mon_to_watersheds1, {
    navpage_name <- "Watersheds"
    updateNavbarPage(session, "pages",navpage_name)
  })
  ...
  observeEvent(input$link_from_wat_ann_to_watersheds1, {
    navpage_name <- "Watersheds"
    updateNavbarPage(session, "pages",navpage_name)
  })
  ...
  observeEvent(input$link_from_wat_ann_to_framework_overview2, {
    panel_name <- "Framework Overview"
    navpage_name <- "Model + Data"
    updateNavbarPage(session, "pages",navpage_name)
    updateTabsetPanel(session, "model_data_panels", panel_name)
  ...
  observeEvent(input$link_from_wat_ann_to_hydro_model_prec, {
    # panel_name <- "Scenario Overview"
    navpage_name <- "Model + Data"
    masterpanel_id <- "model_data_panels"
    panel1_id <- "model_data_hydro_model"
  ...
  observeEvent(input$link_from_scenario_to_forest_disturbances, {
    panel_name <- "Forest Disturbances"
    updateTabsetPanel(session, "model_data_panels", panel_name)
  })
  ...
  # output$active <- renderUI(input$model_data_panels)
...
## ----------------------------------References and Source Code html outputs---------------------------------------## ----
  ...
  output$spatial_analysis_livneh_et_al_2015b <- renderText((references_and_code$livneh_et_al_2015b))
  ...
  output$wat_mon_analysis_livneh_et_al_2015b <- renderText((references_and_code$livneh_et_al_2015b))
  ...
  output$hydro_overview_liang_el_al_1994 <- renderText((references_and_code$liang_el_al_2018))
  ...
  output$framework_liang_et_al_1994 <- renderText((references_and_code$liang_el_al_2018))
  ...
  output$framework_livneh_et_al_2015a <- renderText((references_and_code$livneh_et_al_2015a))
      ...
      if (identical(input$wat_ann_analyses, "wat_ann_analysis_1")) {
        ...
        output$wat_ann_analysis_description<- renderUI({
          tagList(
            ...
            tags$li(
              ...
              actionLink("link_from_wat_ann_to_land_cover","FORE-SCE products"),
              HTML(").")
            ),
        ...
        output$wat_ann_analysis_description<- renderUI({
          tagList(
            ...
            tags$li(HTML("Overall, climate change had a stronger effect on <i>ET</i> than that of any LULC + Forest Disturbance scenario, indicating that <code>climate uncertainty overwhelms the impact of forest disturbance on future <i>ET</i></code>.")),
            br(),
            HTML("<b>Forest Disturbance Impacts under Climate Change:</b>"),
            tags$li(HTML("Overall, forest disturbances slightly reduced <i>ET</i> relative to no disturbance, but differed in their magnitude based on the amount of <i>P</i> in each climate case.")),
            ...
            tags$li(HTML("<code>Forest disturbance impacts to ET were more limited under the Hot/Dry climate"),
                    HTML("due to the more water-limited conditions</code> ("),
        ...
        output$wat_ann_analysis_description<- renderUI({
          tagList(
            ...
            actionLink("link_from_wat_ann_to_framework_overview2",HTML("the total runoff (<i>R</i>) + baseflow (<i>BF</i>).")),
            tags$li(
            ...
            tags$li(HTML("<code>This was attributed to the water-limited conditons of the Hot/Dry case</code>, under which the majority of <i>P</i> was lost to <i>ET</i> regardless of vegetation status.")),
          )
        })
...
## ----------------------------------Spatial-Analyses tab---------------------------------------## ----
    ...
    if (identical(input$spatial_guided_analyses, "spatial_analysis_1")) {
      ...
      output$spatial_analysis_description<- renderUI({
        tagList(
          ...
          tags$li(HTML("These maps show the difference in mean annual values between the case with 30% forest disturbance and the case without disturbance, highlighting where disturbances had the greatest impact.")),
          ...
          tags$li(HTML("Impacts were larger under the more energy-limited Warm/Wet case than the more water-limited Hot/Dry case (explored more in analysis step 4).")),
      ...
      output$spatial_analysis_description<- renderUI({
        tagList(
          ...
          tags$li(HTML("Total runoff and baseflow (<i>RBF</i>) represents the ‘streamflow supplies’ of the system, as the"),
                  actionLink("link_from_spatial_to_framework_overview2",HTML("model routes <i>RBF</i> through the channel network to simulate streamflow (<i>Q</i>) at each outlet."))),
      ...
      output$spatial_analysis_description<- renderUI({
        tagList(
          ...
          tags$li(HTML("<code>This indicated that forest disturbance impacts were smaller in the more water-limited Hot/Dry climate (most of <i>P</i> was lost to <i>ET</i> regardless of vegetation status).</code>")),
          ...
          tags$li(HTML("Impacts to <i>RBFE</i> were more moderate and more variable across the domain in the Cool-Season and included small <i>RBFE</i> increases in the northeast due to larger <i>ET</i> declines in both Warm/Wet and Hot/Dry cases.")),
  ...
  observeEvent(input$spatial_analysis_conclusions_update1, {
    updateVarSelectInput(session,"spatial_var_selected_1",selected = "Efficiency ([runoff + baseflow]/precipitation)")
    updateVarSelectInput(session,"spatial_var_selected_2",selected = "Flow supply (runoff + baseflow)")
    updateVarSelectInput(session,"spatial_temporal_scale_selected",selected = "Annual (Oct-Sep)")
    updateVarSelectInput(session,"spatial_impact_type_selected",selected = "30% Forest disturbance")
  })
  ...
  observeEvent(input$spatial_analyses_panels, {
    ...
    if (input$spatial_analyses_panels=="Overarching Research Conclusions") {
      ...
      output$spatial_analysis_conclusions_description<- renderUI({
        tagList(
          tags$li(HTML("Forest reductions augmented snowpacks and reduced evapotranspiration across the Colorado River Basin under climate change, leading to"),
                  actionLink("link_from_spatial_to_wat_ann2","increased streamflow relative to the case without disturbance.")),
          tags$li(
            actionLink("link_from_spatial_to_wat_mon3","Snowmelt increases in winter months were the primary cause for streamflow increase"),
            HTML(", which is supported by prior work in the region<sup>1-3</sup> and by observed increases in ground snowpack following forest disturbances across North America.<sup>4-8</sup>")),
          ...
          tags$li(actionLink("link_from_spatial_to_wat_ann3","Forest reductions were sufficient to reverse warming impacts to streamflow "),
                  HTML("from key headwater regions in the Warm/Wet climate, "),
                  actionLink("spatial_analysis_conclusions_update1","but not the Hot/Dry case due to more water-limited conditions.")),
          tags$li(HTML("<code>As a result, the climate change uncertainty overwhelms the impact of forest disturbances on future streamflow.</code>")),
          br(),
          helpText(
            HTML("<b>References:</b>"),
            tags$ol(
              tags$li(htmlOutput("spatial_analysis_buma_and_livneh_2015")),
              tags$li(htmlOutput("spatial_analysis_livneh_et_al_2015b")),
              tags$li(htmlOutput("spatial_analysis_moreno_et_al_2016")),
              tags$li(htmlOutput("spatial_analysis_boon_2007")),
              tags$li(htmlOutput("spatial_analysis_zou_et_al_2010")),
              tags$li(htmlOutput("spatial_analysis_biederman_et_al_2015")),
              tags$li(htmlOutput("spatial_analysis_brown_et_al_2014")),
              tags$li(htmlOutput("spatial_analysis_harpold_et_al_2014")),
              tags$li(htmlOutput("spatial_analysis_williams_et_al_2022"))
            )
          )
        )
      })
      ...
      output$spatial_analysis_implications_description<- renderUI({
        tagList(
          ...
          tags$li(HTML("While forest management such as prescribed fires and forest thinning could partially offset the impacts of a Hot/Dry future, these would require large-scale, expensive, and sustained treatments that affect other ecosystem services.<sup>2</sup>")),
          tags$li(HTML("It is important to consider the"),
                  ...
                  HTML("the streamflow benefits from permanent forest disturbance are idealized scenarios that do not account for forest recovery and its long-term impact on runoff.<sup>3</sup>")),
    ...
    if (identical(input$wat_mon_analyses, "wat_mon_analysis_1")) {
      ...
      output$wat_mon_analysis_description<- renderUI({
        tagList(
          ...
          tags$li(HTML("<i>M</i> also increased during spring to early summer months due to the higher ground snowpack.")),
      ...
      output$wat_mon_analysis_description<- renderUI({
        tagList(
          ...
          tags$li(HTML("<code>Forest disturbances in the Hot/Dry case were sufficient to offset warming in some months, but not on an"), 
                  ...
                  HTML("(due to more water-limited conditions).</code>")),
      ...
      output$wat_mon_analysis_description<- renderUI({
        tagList(
          HTML("<b>Conclusions:</b>"),
          tags$li(HTML("Snowmelt increases in winter months were the primary cause for streamflow increase, which is supported by prior work in the region<sup>1-3</sup> and by observed increases in ground snowpack following forest disturbances across North America.<sup>4-8</sup>")),
          tags$li(HTML("The increases in water yield scaled with the forest disturbance amounts, confirming similar conclusions of prior efforts.<sup>9</sup>")),
          tags$li(HTML("Forest reductions of 60% or more were sufficient to reverse warming impacts to streamflow in key months in the Warm/Wet case.")),
          ...
          helpText(
            ...
            tags$ol(
              ...
              tags$li(htmlOutput("wat_mon_analysis_livneh_et_al_2015b")),
